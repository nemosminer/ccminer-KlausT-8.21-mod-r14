...
{
	G(s[0], s[1], s[2], s[3]);
	s[1] = shuffle2(0xFFFFFFFF, s[1], threadIdx.x + 1, 4);
	s[2] = shuffle2(0xFFFFFFFF, s[2], threadIdx.x + 2, 4);
	s[3] = shuffle2(0xFFFFFFFF, s[3], threadIdx.x + 3, 4);
	G(s[0], s[1], s[2], s[3]);
	s[1] = shuffle2(0xFFFFFFFF, s[1], threadIdx.x + 3, 4);
	s[2] = shuffle2(0xFFFFFFFF, s[2], threadIdx.x + 2, 4);
	s[3] = shuffle2(0xFFFFFFFF, s[3], threadIdx.x + 1, 4);
}
	...
	if ((thread + offset) < threads)
	{
		...
		for (row = 2, prev = 1, rowa = 0, step = 1, window = 2, gap = 1; row < nRows; row++) {
			...
			for (uint32_t i = 0; i < nCols; i++) {
				...
				bufstate[0] = shuffle2(0xFFFFFFFF, state[0], threadIdx.x + 3, 4);
				bufstate[1] = shuffle2(0xFFFFFFFF, state[1], threadIdx.x + 3, 4);
				bufstate[2] = shuffle2(0xFFFFFFFF, state[2], threadIdx.x + 3, 4);
...
void lyra2_gpu_hash_32_2_shared_4way(uint32_t threads, uint32_t offset, const uint32_t timeCost, const uint32_t nRows, const uint32_t nCols)
{
	...
	if ((thread + offset) < threads)
	{
		...
		for (uint32_t tau = 1; tau <= timeCost; tau++)
		{
			...
			do {
				rowa = (uint32_t)(devectorize(shuffle2(0xFFFFFFFF, state[0], 0, 4)) % (uint64_t)nRows);
...
void lyra2_gpu_hash_32_3_16way(uint32_t threads, uint32_t offset, const uint32_t timeCost, const uint32_t nRows, const uint32_t nCols, uint32_t tau, uint32_t prev)
{
	...
	if ((thread + offset) < threads)
	{
		...
		do {
			rowa = (uint32_t)(devectorize(shuffle2(0xFFFFFFFF, state, 0, 16)) % (uint64_t)nRows);
...
void lyra2_cpu_init(int thr_id, uint32_t threads, const uint32_t nRows, const uint32_t nCols, uint64_t *d_state)
{
	...
	if ((cuda_arch[dev_id] > 500 && nRows*nCols > 64) || (cuda_arch[dev_id] == 500 && nRows*nCols > 42)) Mode[thr_id] = GLOBAL_16WAY_MODE;
...
void lyra2_cpu_hash_80(int thr_id, uint32_t threads, uint32_t startNounce, const uint32_t timeCost, const uint32_t nRows, const uint32_t nCols, const uint32_t bug, uint64_t *Hash)
{
	...
	case GLOBAL_4WAY_MODE:
		for (uint32_t i = 0; i < 4; i++) {
			...
			lyra2_gpu_hash_32_2_4way << < grid, block3 >> > (threads, grid.x * 8 * i, timeCost, nRows, nCols);
			for (uint32_t tau = 1; tau <= timeCost; tau++) {
				lyra2_gpu_hash_32_3_4way << < grid, block3 >> > (threads, grid.x * 8 * i, timeCost, nRows, nCols, tau, prev);
	...
	case GLOBAL_16WAY_MODE:
		for (uint32_t i = 0; i < 16; i++) {
			uint32_t prev = nRows - 1;
			lyra2_gpu_hash_32_2_16way << < grid, block2 >> > (threads, grid.x * 2 * i, timeCost, nRows, nCols);
			for (uint32_t tau = 1; tau <= timeCost; tau++) {
				lyra2_gpu_hash_32_3_16way << < grid, block2 >> > (threads, grid.x * 2 * i, timeCost, nRows, nCols, tau, prev);
				prev = (tau & 1) == 0 ? 1 : (nRows >> 1) + 1;
			}
		}
		break;
