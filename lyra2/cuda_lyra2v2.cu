...
#define shuffle(d, a, b, c) d = __shfl_sync(0xFFFFFFFF, a, b, c)

#define shuffle6(d0, d1, d2, a0, a1, a2, b0, b1, b2, c) { \
	d0.x = __shfl_sync(0xFFFFFFFF, a0.x, b0, c); \
	d0.y = __shfl_sync(0xFFFFFFFF, a0.y, b0, c); \
	d1.x = __shfl_sync(0xFFFFFFFF, a1.x, b1, c); \
	d1.y = __shfl_sync(0xFFFFFFFF, a1.y, b1, c); \
	d2.x = __shfl_sync(0xFFFFFFFF, a2.x, b2, c); \
	d2.y = __shfl_sync(0xFFFFFFFF, a2.y, b2, c); \
}
#else
#define bitselect(a, b, c) ((a) ^ ((c) & ((b) ^ (a))))
#define shuffle(d, a, b, c) { \
	extern __shared__ uint32_t shared_mem[]; \
	shared_mem[0 * TPB + threadIdx.x] = a; \
	__threadfence(); \
	d = shared_mem[0 * TPB + bitselect(threadIdx.x, b, c)]; \
}
...
#define shuffle6(d0, d1, d2, a0, a1, a2, b0, b1, b2, c) { \
	...
	__threadfence(); \
	d0.x = shared_mem[0 * TPB + bitselect(threadIdx.x, b0, c)]; \
	d0.y = shared_mem[1 * TPB + bitselect(threadIdx.x, b0, c)]; \
	d1.x = shared_mem[2 * TPB + bitselect(threadIdx.x, b1, c)]; \
	d1.y = shared_mem[3 * TPB + bitselect(threadIdx.x, b1, c)]; \
	d2.x = shared_mem[4 * TPB + bitselect(threadIdx.x, b2, c)]; \
	d2.y = shared_mem[5 * TPB + bitselect(threadIdx.x, b2, c)]; \
}
#endif
...
#define round_lyra_4way(ds0, ds1, ds2, ds3) { \
	...
	shuffle6(ds1, ds2, ds3, ds1, ds2, ds3, threadIdx.x + 1, threadIdx.x + 2, threadIdx.x + 3, 4); \
	Gfunc(ds0, ds1, ds2, ds3); \
	shuffle6(ds1, ds2, ds3, ds1, ds2, ds3, threadIdx.x + 3, threadIdx.x + 2, threadIdx.x + 1, 4); \
}
...
#define reduceDuplexRowSetup2(s0, s1, s2, n, r) { \
	...
	shuffle6(Data0, Data1, Data2, state[0], state[1], state[2], threadIdx.x + 3, threadIdx.x + 3, threadIdx.x + 3, 4); \
...
void lyra2v2_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint64_t *g_hash)
{
	...
	if (device_sm[dev_id] >= 750) { tpb1 = TPB75; sm = TPB75 * 128; }
	else if (device_sm[dev_id] > 600) { tpb1 = TPB61; sm = TPB61 * 192; }
	else if (device_sm[dev_id] == 600) { tpb1 = TPB60; sm = TPB60 * 128; }
